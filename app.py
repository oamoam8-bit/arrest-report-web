...
with st.container(border=True):
    ...
    # บล็อคที่ 1: ร่างพฤติการณ์ใหม่ (อัปเกรดคำสั่งเป็นฟอร์มบันทึกจับกุม 100%)
    ...
    if st.button("📝 ร่างพฤติการณ์จากข้อมูลทั้งหมด"):
        if not charge_input or not evidence or not arrest_loc or not suspect_name:
            ...
            try:
                ...
                # เปลี่ยน Prompt ใหม่ทั้งหมด บังคับรูปแบบการเขียนให้เป็นบันทึกจับกุม
                prompt = (
                    f"คุณคือพนักงานสืบสวนผู้เชี่ยวชาญการทำเอกสาร 'บันทึกการจับกุม' ของตำรวจไทย\n"
                    f"จงนำข้อมูลด้านล่างนี้ไปเขียนบรรยายในหัวข้อ 'พฤติการณ์ในการจับกุม' เพื่อนำไปแทรกในฟอร์มบันทึกจับกุมทันที\n\n"
                    f"ข้อห้ามเด็ดขาด (Strict Rules):\n"
                    f"1. ห้ามเขียนในรูปแบบการรายงานเหตุถึงผู้บังคับบัญชาเด็ดขาด (ห้ามมีคำว่า เรียน, เรื่อง, กระผม)\n"
                    f"2. ห้ามใช้คำแทนตัวว่า ผม หรือ ข้าพเจ้า ให้ใช้คำว่า 'เจ้าพนักงานตำรวจชุดจับกุม'\n"
                    f"3. ห้ามเว้นช่องว่าง [ ] เพื่อให้เติมเองเด็ดขาด ให้นำข้อมูลที่มีไปแต่งเรื่องให้เนียนที่สุด\n\n"
                    f"ข้อมูลที่ต้องใช้ผูกเป็นเรื่องราว:\n"
                    f"- วันเวลาจับกุม: {arrest_date}\n"
                    f"- สถานที่เกิดเหตุ: {arrest_loc}\n"
                    ...
                    f"รูปแบบการบรรยาย:\n"
                    f"ให้เริ่มประโยคบรรยายเหตุการณ์เลย เช่น 'ก่อนทำการจับกุมในคดีนี้ เจ้าพนักงานตำรวจชุดจับกุมได้รับแจ้ง...' หรือ 'ตามวันเวลาเกิดเหตุ เจ้าพนักงานตำรวจชุดจับกุมได้ออกปฏิบัติหน้าที่...' "
                    f"จากนั้นเล่าลำดับเหตุการณ์ไปจนถึงการเดินทางไปถึงสถานที่เกิดเหตุ การพบตัวผู้ต้องหา การแสดงตัวเป็นเจ้าพนักงานตำรวจ การตรวจค้นพบของกลาง การแจ้งข้อกล่าวหาและแจ้งสิทธิตามกฎหมาย "
                    f"และจบประโยคสุดท้ายด้วยการควบคุมตัวผู้ต้องหาพร้อมของกลางส่งพนักงานสอบสวนเพื่อดำเนินคดีตามกฎหมาย"
                )
                
                with st.spinner('กำลังร่างพฤติการณ์ตามรูปแบบบันทึกจับกุม (รอสักครู่)...'):
    ...
    if st.button("✨ เกลาพฤติการณ์ให้สมบูรณ์พร้อมใช้"):
        if not behavior_input:
            ...
            try:
                ...
                prompt = (
                    f"คุณคือพนักงานสืบสวนผู้เชี่ยวชาญการทำเอกสาร 'บันทึกการจับกุม' ของตำรวจไทย\n"
                    f"จงนำเหตุการณ์ย่อต่อไปนี้:\n'{behavior_input}'\n\n"
                    f"มาเขียนขยายความและเกลาเป็น 'พฤติการณ์การจับกุม' ฉบับสมบูรณ์ เพื่อแทรกในฟอร์มบันทึกจับกุม\n"
                    f"ข้อห้ามเด็ดขาด:\n"
                    f"1. ห้ามเขียนในรูปแบบรายงานผู้บังคับบัญชา (ห้ามมีคำว่า เรียน, เรื่อง, กระผม)\n"
                    f"2. ห้ามใช้คำแทนตัวว่า ผม หรือ ข้าพเจ้า ให้ใช้คำว่า 'เจ้าพนักงานตำรวจชุดจับกุม'\n"
                    f"3. ห้ามเว้นช่องว่าง [ ] ให้เติมเองเด็ดขาด\n\n"
                    f"โดยบังคับให้นำข้อมูลต่อไปนี้แทรกเข้าไปในเนื้อเรื่องให้ครบถ้วนและแนบเนียน:\n"
                    ...
                    f"ข้อหา: {charge_input}\n"
                    f"คำให้การ: {suspect_statement}\n\n"
                    f"ปรับสำนวนให้เป็นภาษากฎหมายที่รัดกุม สละสลวย ตามหลักการเขียนบันทึกจับกุม เล่าตั้งแต่การเข้าจับกุมจนถึงการนำส่งพนักงานสอบสวน"
                )
